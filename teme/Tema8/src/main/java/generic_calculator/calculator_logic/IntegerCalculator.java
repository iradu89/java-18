...
public class IntegerCalculator implements Calculator<Integer> {
    ...
    public Integer calculate(List<String> inputList) {
    ...
    private int secondAndThirdOrderOfOperations(List<String> list) {
    ...
    private void resolveParentheses(List<String> list, int startingIndex, int endIndex) {
        ...
        int result = secondAndThirdOrderOfOperations(parenthesesList);
    ...
    private void secondOrderOfOperations(List<String> list) {
        ...
        while (inputIterator.hasNext()) {
            ...
            switch (element) {
                case "*": {
                    int[] values = getValuesAndModifyList(inputIterator);
     ...
     */
    private int thirdOrderOfOperations(List<String> list) {
        for (int i = 0; i < list.size(); i++) {
            switch (list.get(i)) {
                case "+":
                    list.set(i + 1, String.valueOf(this.add(Integer.valueOf(list.get(i - 1)), Integer.valueOf(list.get(i + 1)))));
                    break;
                case "-":
                    list.set(i + 1, String.valueOf(this.subtract(Integer.valueOf(list.get(i - 1)), Integer.valueOf(list.get(i + 1)))));
                    break;
            }
        }
    ...
    */
    private int[] getValuesAndModifyList(ListIterator<String> inputIterator) {
        int[] result = new int[2];
        inputIterator.previous();
        result[0] = Integer.parseInt(inputIterator.previous());
        inputIterator.remove();
        inputIterator.next();
        inputIterator.remove();
        result[1] = Integer.parseInt(inputIterator.next());
        return result;
    }
}
